...
;(function (factory) {
  ...
  S2.define('select2/selection/base',[
    ...
    BaseSelection.prototype._attachCloseHandler = function (container) {
      ...
      $('.lpe-popup-config-submit-form .content-popup-submit-form').on('mousedown.select2.' + container.id, function (e) {
        var $target = $(e.target);
    ...
    BaseSelection.prototype._detachCloseHandler = function (container) {
      $('.lpe-popup-config-submit-form .content-popup-submit-form').off('mousedown.select2.' + container.id);
    };
  ...
  S2.define('select2/dropdown/attachBody',[
    ...
    function AttachBody (decorated, $element, options) {
      this.$dropdownParent = $(options.get('dropdownParent') || '.lpe-popup-config-submit-form .content-popup-submit-form');
...
$(document).ready(function(){
    ...
    var showPopupSettingApps = function(params){
                                ...
                                '<div class="content-popup-submit-form">' +
                                    ...
                                    '<div class="content-popup">' +
                                        '<div class="header-steps">' +
                                            '<div class="item step1" data-name="step1">' +
                                                '<div class="title">' +
                                                    '<span class="number">1</span>' +
                                                    '<span class="title_text">Danh sách</span>' +
                                                '</div>' +
                                            '</div>' +
                                            '<div class="item step2" data-name="step2">' +
                                                '<div class="title">' +
                                                    '<span class="number">1</span>' +
                                                    '<span class="title_text">Thông tin cấu hình</span>' +
                                                '</div>' +
                                            '</div>' +
                                            '<div class="item step3" data-name="step3">' +
                                                '<div class="title">' +
                                        ...
                                        '<div class="item-content" data-name="email">' +
                                            ...
                                            '<div class="item-step step2" data-step="step2">' +
                                                '<div class="content-form-conffig">' +
                                                    '<form>' +
                                                        '<div class="form-group">' +
                                                            '<label for="api_key" class="form-label">Api Key</label>' +
                                                            '<input type="text" name="api_key" id="api_key" placeholder="Api Key" class="valid" aria-invalid="false">' +
                                                        '</div>' +
                                                        '<div class="form-group">' +
                                                            '<label for="api_secret_key" class="form-label">Api Secret Key</label>' +
                                                            '<input type="text" name="api_secret_key" id="api_secret_key" placeholder="Api Secret Key" class="valid" aria-invalid="false">' +
                                                        '</div>' +
                                                        '<div class="form-group">' +
                                                            '<label for="project_key" class="form-label">Project Key</label>' +
                                                            '<input type="text" name="project_key" id="project_key" placeholder="Project Key" class="valid" aria-invalid="false">' +
                                                        '</div>' +
                                                    '</form>' +
                                                '</div>' +
                                            '</div>' +
        ...
        var switchStepContentByTab = function(tabActiveFn){
            ...
            var $headerStep = $($popup).find('.header-steps');
        ...
        if($itemApp && $itemApp.length > 0){
            $($itemApp).off('click.choseApp').on('click.choseApp', function(){
                ...
                if(nameTab){
                    ...
                    var nameApp = $(this).attr('data-name');
                    if(options[nameTab]){
                        options[nameTab].step1.data = nameApp
                    }
                }
            });
        }

        var $itemStepHeader = $($popup).find('.header-steps .item');
        if($itemStepHeader && $itemStepHeader.length > 0){
            $($itemStepHeader).off('click.swichStepHeader').on('click.swichStepHeader', function(){
                var nameStep = $(this).attr('data-name');
                if(nameStep == 'step2'){
                    options[options.tabActive].step1.position = false;
                            
                    //set lại step2
                    options[options.tabActive].step2.position = true;
                    
                    //set lại step3
                    options[options.tabActive].step3.position = false;

                    switchStepContentByTab(options.tabActive);
                }else if(nameStep == 'step3'){
                    options[options.tabActive].step1.position = false;
                            
                    //set lại step2
                    options[options.tabActive].step2.position = false;
                    
                    //set lại step3
                    options[options.tabActive].step3.position = true;
